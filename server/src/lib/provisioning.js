...
const db = require('../db');
const { normalizePhone } = require('./identifier');
 ...
 * @throws {Error} with `.status` set when the request can't be provisioned
 *                 (already reviewed, or the email/phone already belongs to an account).
 */
async function provisionAccountFromRequest(request, overrides = {}) {
  ...
  if (existing) {
    const error = new Error(
      ...
      'Share a password reset instead of creating a duplicate.'
    );
    error.status = 409;
    throw error;
  }
...
module.exports = { provisionAccountFromRequest, generateTemporaryPassword };
